 ...
 */
export function stringTemplate(str: string, replacements: Record<string, string>): string {
    const repl = objKeysToLowercase(replacements);
    return str.replace(/\{\{\s*([0-9a-zA-Z_]+)\s*\}\}/g, (match, key_) => {
        const key = key_.toLowerCase();
        const value = Object.prototype.hasOwnProperty.call(repl, key) ? repl[key] : null;
        console.log(key_, value, repl[key], Object.prototype.hasOwnProperty.call(repl, key));
